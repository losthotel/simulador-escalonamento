...
def escalonador_round_robin(processos, quantum):
    ...
    while fila:
        ...
        if processo.tempo_restante > quantum:
            saida += f"Executando {processo.nome} por {quantum} unidades de tempo.\n"
            processo.tempo_restante -= quantum
            tempo_total += quantum
            fila.append(processo)
        else:
            saida += f"Executando {processo.nome} por {processo.tempo_restante} unidades de tempo. {processo.nome} concluído.\n"
...
def executar():
    ...
    quantum = 3
    return escalonador_round_robin(processos, quantum)
